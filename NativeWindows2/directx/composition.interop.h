...
{
    auto surface = graphics.CreateDrawingSurface({w, h}, pixelfmt, alpha);
...
static __forceinline void ResizeD2dSurface(IN CompositionDrawingSurface& surface, LONG width, LONG height)
{
...
static __forceinline void ResizeD2dSurface(IN CompositionVirtualDrawingSurface& surface, LONG width, LONG height)
{
    width = std::max(1l, width);
    height = std::max(1l, height);
...
static __forceinline auto CreateCompositionSurfaceForHandle(
    Compositor const& compositor, HANDLE handle)
{
    ICompositionSurface surface{ nullptr };
    auto compositorInterop = compositor.as<abicomp::ICompositorInterop>();
    com_ptr<abicomp::ICompositionSurface> surfaceInterop;

    check_hresult(compositorInterop->CreateCompositionSurfaceForHandle(handle, surfaceInterop.put()));
...
static __forceinline auto CreateCompositionSurfaceForHandle(
    abicomp::ICompositorInterop* compositorInterop, HANDLE handle)
{
    ICompositionSurface surface{ nullptr };
